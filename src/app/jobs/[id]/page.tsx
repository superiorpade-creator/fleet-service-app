...
export default async function JobDetailPage({ params }: { params: { id: string } }) {
  ...
  // Fetched for everyone now (crew need the site address to find the
  // job), but which fields actually get shown below is still gated by
  // role - crew only ever see the address, never contact info.
  let customer: Customer | null = null;
  if ((job as Job).customer_id) {
  ...
  return (
    <>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 pb-24">
        ...
        {/* Admin-only: full contact info is not exposed to crew */}
        {isAdmin && customer && (customer.contact_name || customer.phone || customer.email || customer.address) && (
        ...
        {/* Crew: address only, no contact name/phone/email/notes */}
        {!isAdmin && customer?.address && (
          <div className="bg-white border border-line rounded-lg p-3 mb-6 text-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-steel mb-1">Site Address</p>
